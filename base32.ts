...
const toBase32 = (input: string | Uint8Array): string => {
  let offset = 0;

  const _toBase32 = (input: Uint8Array): string => {
    let output = "";
  ...
  const _getNextGroup = (input: Uint8Array): { encoded: Uint8Array, numberOfBytes: number } => {
    ...
    switch (offset - input.length) {
      ...
      case 4: numberOfBytes = 7; break;
      default: numberOfBytes = 8; break;
    }
